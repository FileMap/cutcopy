...
Napi::Boolean isCutJs(const Napi::CallbackInfo &info)
{
    auto env = info.Env();
    int b = isCut();
    ...
    if (b == -1)
    {
        ...
        // Napi::TypeError::New(env, "Can't determine the operation, between cut and paste.")
        //     .ThrowAsJavaScriptException();
    }

    return Napi::Boolean::New(env, b==1);
}

void changeDropEffectJs(const Napi::CallbackInfo &info)
{
    auto env = info.Env();
    if (info.Length() < 1)
    {
    ...
    int option = info[0].As<Napi::Number>();
    if (option < 0 || option > 2)
    {
        Napi::TypeError::New(env, "Expect dropEffect to be 0, 1 or 2. They stand for: none, copy, move")
            .ThrowAsJavaScriptException();
        return;
    }

    changeDropEffect(option);
    return;
}
...
Napi::Object Init(Napi::Env env, Napi::Object exports) {
    exports.Set("isCut", Napi::Function::New(env, isCutJs));
    exports.Set("changeDropEffect", Napi::Function::New(env, changeDropEffectJs));
